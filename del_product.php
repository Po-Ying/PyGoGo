...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 删除与该用户相关联的购物车项目
    $query_delete_cart = "DELETE FROM cart WHERE product_id=?";
    $stmt_delete_cart = mysqli_prepare($link, $query_delete_cart);
    if ($stmt_delete_cart) {
        mysqli_stmt_bind_param($stmt_delete_cart, "i", $product_id);
    ...
    // 删除用户信息
    $query_delete_user = "DELETE FROM products WHERE id=?";
    $stmt_delete_user = mysqli_prepare($link, $query_delete_user);
    if ($stmt_delete_user) {
        mysqli_stmt_bind_param($stmt_delete_user, "i", $product_id);
